...
create a doubly-linkedlist of nodes ordered from least recent to most.

nodes contain info like use counter, key and value.

get operation should update the use counter of the node and return value of it, moving it to back of linkedlist.

put operation:

if new value:
    if not at capacity, put to back of LL and add to nodes table
    
    otherwise get nodes that have the lowest use-counter.
    if there are multiple, remove the least recently used one from the LL

else:
    update value, increase counter and put to back of LL


2 tables:
key - node

frequency - set of nodes
